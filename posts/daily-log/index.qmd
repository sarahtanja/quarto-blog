...
-    Met with Kip, Added Kip to WDFW scientific collection permit
    ...
    -   9 permit locations!
    - Messing around with mapping the collection sites, from R Graph Gallery, https://r-graph-gallery.com/180-change-background-in-leaflet-map.html
...
```{r}
# make data file with lat/lon coordinates of collection locations
sites <- c()

# bounding box for puget sound
puget <- c(bottom = 47.3, top = 48.8, right = -122.2, left = -122.8)

# make base maps using stamen maps
pugetmap <- get_stamenmap(bbox = puget, zoom = 14,
              maptype = "toner", crop = FALSE)
ggmap(pugetmap)

# add points to base raster map
ggmap(pugetmap) +
  geom_point(data = pseafarm, mapping = aes(x = lon, y = lat, 
                                            color = name, shape = factor(permit)),
                                  alpha = 0.8,
                                  size = 3) 
...
gg <- gg + coord_sf(xlim=c(-125, -122),  # WA
                    ylim=c(46, 48))
gg <- gg + theme_map()
gg
...
Interactive Map with `GGPLOT2` and `
-   scheduled SAFS truck for Friday FEB 23 from 10-4 for seawater transfer
