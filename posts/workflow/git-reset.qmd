...
**`.gitignore` cannot filter files by size** — it only filters by **path
patterns**, like filenames, extensions, or directory names. It doesn’t
support dynamic criteria like file size, modification time, etc.
...
2.  `find . -type f -size -50M -exec git add {} +`
    ...
    3.  `-size -50M`: Limits the size to files smaller than 50
        megabytes.
    4.  `-exec git add {} +`: Executes the `git add` command on all
        found files, adding them to the Git staging area.
3.  `git status` : run again to double check staged, not staged, and
    untracked files
4.  `git commit -m "your message here"`
5.  `git pull` : merge if needed, see [Write & quit vim during a
    merge](#write-quit-vim-during-a-merge)
6.  `git push`

# Write & quit vim during a merge {#write-quit-vim-during-a-merge}

1.  Press `INS` key to insert text, write a short message about the
    merge
